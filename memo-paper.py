...
# メモ帳の大きさなどレイアウトを指定
layout = [
    [sg.Multiline(size=(50,30),key="text")],# 行数と列数を指定し、書き込まれるデータをtxt形式に限定
    [sg.Button("SAVE"),sg.Button("OPEN")],# 保存用とフォルダを開く用のボタンを作成 
 ]
#表示するウィンドウを作成
...
while True:
    ...
    if event == "OPEN":
        ...
        if not os.path.exists(SAVE_FILE):
            sg.popup("Not Saved")
            continue
        # ファイルが存在する場合指定したファイルに書き込みを行う。書き込みの場合はr(rite)を指定
