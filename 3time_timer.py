...
# Mp3を再生するための事前準備
pg.mixer.init()
pg.mixer.music.load(SOUND_FILE) #変数を使い代入されている音声ファイルを読み込む
# タイマーのレイアウトを指定
Layout = [
...
while True:
    ...
    #リセットボタンを押した際の挙動
    if event == "Reset":
        # start_time変数の値をNoneにして最初の時間(3分)に戻す
        start_time = None
    ...
    #タイマー開始時間から現在時刻の差分が設定した時間(3分)を超えていた場合の処理
    ...
    window["-output-"].update(
        "0" + str(da.timedelta(seconds=remain)))
window.close()