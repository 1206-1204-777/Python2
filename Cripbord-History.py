...
# lambda関数（ラムダ関数、無名関数と呼ばれる簡単な処理を一時的に行うための関数）を使い各処理を定義
    ...
    short = lambda v: v[:20] + "..." if len(v) > 20 else v  # 表示する文字数を20文字までに限定。20文字を超える場合以降の文字を"..."に置き換えて表示(保存した内容が変わるわけではない)
    return [f"{i+1:02}: {lf(short(h))}" for i, h in enumerate(history)] # 上記で定義したlambda関数を使い整形、履歴にインデックス(行番号)を付与しhistoryに再格納する
